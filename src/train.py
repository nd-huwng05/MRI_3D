...
def train(args):
    ...
    if hasattr(args, "resume_path") and args.resume_path and os.path.exists(args.resume_path):
        if os.path.isfile(args.resume_path):
            ...
            if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
                ...
                print("Loaded checkpoint from: ", args.resume_path, "but will train with new optimizer", "Start epoch: ", start_epoch)
        else:
    ...
    print(f"Starting training form {start_epoch + 1} to {args.epochs} epochs...")
    for epoch in range(start_epoch, args.epochs):
        model.train()
        pbar = tqdm(dataloader, desc=f"[Train] Epoch {epoch+1}/{args.epochs}")

        total_vae_loss = 0
        total_diff_loss = 0

        for batch in pbar:
            images = batch['target'].to(device) # Shape: [B, 3, 256, 256]
            B = images.shape[0]
            ...
            kld_loss = -0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp()) / (B * 256 * 256) # Kullback-Leibler Divergence
        ...
        print(f"Epoch {epoch + 1}: Pixel-AUROC: {val_p_auroc:.4f} DICE: {val_dice:.4f} Img-AUROC: {val_img_auroc:.4f} Val_IOU: {val_iou:.4f} vae-loss: {avg_vae_loss:.4f}, diff-loss: {avg_diff_loss:.4f}")
...
if __name__ == "__main__":
    parser = ArgumentParser(description='Training model Latent Anomaly Diffusion Network')
